...
% UTORid>.
%% Student Information
% Student Name: Adam Unus Yanni Omarali
% 
...
y = zeros(n, 1);
for j = 1:n
    x(j) = a + (j-1) * (b - a) / (n - 1); %start at the initial value a, then increase by evenly divded steps
end
...
t = linspace(1, 5, 100);
e = exp(1);
sol1 = 1./exp(t.^2) .* (t-e-1);
sol2 = 1./exp(t.^2) .* (t-1);
sol3 = 1./exp(t.^2) .* (t+e-1);
plot(t, sol1, t, sol2, t, sol3)
xlabel("t");
ylabel("y");
title('Particular Solutions of Given ODE');
legend('solution for y(1)=-1', 'solution for y(1)=0','solution for y(1)=1');
...
t = linspace(-5, 5, 100);
plot(t, f(t))
title('Plot of inline function f(t)');
xlabel("t");
ylabel("f");
...
guess = 1;
for i=1:100
    yvals(i) = fzero(@(y) f(xvals(i), y) - c, guess);
end
plot(xvals, yvals)
title('Plot of Implicit Equation');
xlabel("x");
ylabel("y");
%% Exercise 10
...
%why tf does this question say be careful of your choice of guess? how are
%we suppose to use fzero
% general = @(x, y) c .* exp(-sin(x) + x.^3) - y
particular = @(x, y) exp(-sin(x) + x.^3) - y;
xvals = linspace(-1.5, 1.25, 100);
yvals = zeros(100, 1);
for i=1:100
    yvals(i) = fzero(@(y) particular(xvals(i), y), 0);
end

plot(xvals, yvals)
title('Plot of Implicit Solution to DE');
xlabel("x");
ylabel("y");
%% Install iode
...
%At y = 0 or x = 0, dy/dx = 0. 

%Above the x-axis, solutions are shaped like a positive porabola. As x
%approaches 0, the solutions asymptope near y = 0.

%Below the x-axis, solutions asymptope at y = 0 as x approaches infinity.
%And as x approaches 0, y approaches negative infinity.

%The solutions are symmetric about the y-axis because plugging x and -x
%into the differential equation just changes the slope from positive to
%negative, but the magnitude stays the same. As a result, the solutions are
...
%Solutions with y0 < -1 will have positive slope (increasing) and approach
%y = -1, where it asymptopes. 

%The other constant solution is where y0>-1, where the curve will decrease
%and approach y = 1, where it asymptopes.

%Solutions can not cross because the direction field determines where the
%solution will go next. The same direction (same derivative) can not result
%in two functions moving in different directions.
