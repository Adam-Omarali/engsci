...
tvals = linspace(t0, t1, 20);
manual = -0.5 .* cos(tvals);
plot(my_soln.x, my_soln.y, tvals, manual, 'x', 'MarkerSize',10, 'LineWidth', 2)
legend('Exact', 'Numerical','Location','Best');
xlabel('t');
ylabel('y');
title('Q1: Exact and Numerical Solution to Linear ODE')
...
yexact = -0.5 .* cos(my_soln.x);
err = abs(yexact - my_soln.y);

fprintf('maximum error: %g \n', max(err));
semilogy(my_soln.x, err, 'LineWidth', 2)
xlabel("t");
ylabel("log of error");
title("Q3: Numerical and Exact Error");

%The error is largest around t = 1.5 because the derivative
%of y is largest (inflection point), meaning the curve is changing at its greatest rate.
%% Exercise 4
...
t0 = 1;
t1 = 10;
my_soln2 = ode45(f2, [t0, t1], y0);

exact = (3.*my_soln2.x -2).^(1./3);


err = abs(exact - my_soln2.y);
fprintf('maximum error: %g \n', max(err));

plot(my_soln2.x, my_soln2.y, my_soln2.x, exact, 'x', 'MarkerSize',10, 'LineWidth', 2)
xlabel('t');
ylabel('y');
legend('Exact', 'Numerical','Location','Best');
title("Q4: Exact and Numerical Solutions to Non-Linear ODE")
%% Exercise 5
...
t0 = 0;
t1 = 10;
my_soln3 = ode45(f3, [t0, t1], y0);
plot(my_soln3.x, my_soln3.y)
xlabel("t");
ylabel("y");
title("Q5: Solution to Non-Solvable ODE")
...
t0 = 0;
t1 = 1;
my_soln4 = ode45(f4, [t0, t1], y0);
plot(my_soln4.x, my_soln4.y)
xlabel("t");
ylabel("y");
title("Q6: ODE That Blows Up to Infinity");

%When plotting the direction field in iode,
%given the initial condition t_0 = 0, y(t_0) = 1, 
%the derivative diverges to infinity before reaching t=1.
%Analysing the form of the ODE, since y_0 > 1, the differential equation
%is dominated by y (since it's cubed) and will increase to inifinity.
...
% Plot the approximation just found
picard=ezplot(phi,[-pi/4,pi/4]);
set(picard,'Color','green');        

hold on;
exact=ezplot(tan(t + pi/4),[-pi/4,pi/4]);

xlabel('t');
ylabel('y');
title('Q8: Picard Approximations');
legend('Picard Approximation', 'Exact Solution','Location','NorthWest');