...

{
	...
	\vspace*{10pt}
	Start by considering the case ${A} = \bf{0}$. Since ${A}$ is a matrix with only zero columns, all columns are linearly dependent. 
	By definition, $dim\{\bf{0}\} = 0$. The rank of $B$ is confined by the following inequality: $0\leq \mathrm{rank}\ B \leq min\{k, n\}$ since 
	the rank of a matrix can be at the most the mininumum of the number of rows and columns as $dim\ col\ B = dim\ row\ B = dim\ \mathrm{rank}\ B$.
	We also know $\mathrm{rank}\ AB = \mathrm{rank}\ {\bf{0}} = dim\{\bf{0}\} = 0$. Subbing these values into the inequality we are given:
	\begin{align*}
		\mathrm{rank}\ {A} + \mathrm{rank}\ B - k &\leq \mathrm{rank} AB \\
		0 + \mathrm{rank}\ B - k &\leq 0 \\
		min\{m , k\}  - k &\leq 0 \\
		min\{m , k\} &\leq k \\
	\end{align*} 
	% Consider the case $n > k$, where the number of columns is greater than the number of rows in $\bfB$. Then $\mathrm{rank}\bfB$ can be atmost $k$.
	% \begin{align*}
	% 	0 + k - k &\leq 0 \\
	% 	0 &\leq 0
	% \end{align*} 
	% The other case is when $n \geq k$, where the number of columns is less than the number of rows in $\bfB$. Then $\mathrm{rank}\bfB$ can be atmost $n$.
	% \begin{align*}
	% 	0 + n - k &\leq 0 \\
	% 	n - k &\leq 0 \\
	% \end{align*}
	% Since $n \geq k$, $n - k \leq 0$. Therefore, the inequality holds true for both cases.
	\\
	Now we consider the case where ${A} \neq {0}$. Since ${A} \neq \bf{0}$, $1 \leq \mathrm{rank}\ {A} \leq min\{m, k\}$.
	If $B = 0$, a similar argument stands for ${A} = 0$.
	\begin{align*}
		\mathrm{rank}\ {A} + \mathrm{rank}\ B - k &\leq \mathrm{rank} AB \\
		\mathrm{rank}\ {A} + 0 - k &\leq 0 \\
		min\{m , k\} &\leq k \\
	\end{align*}
...

{
	...
	We know $AB \in {^m}\mathbb R^n$. Using the rank factorization $AB = CD$, $CD$ must also be in ${^m}\mathbb R^n$. 
	Since $C$ forms a basis for $AB$, it must have $rank AB = r$ independent columns. To perform matrix multiplication, $D$ must have $r$ rows. 
	Therefore, $C \in {^m}\mathbb R^r$ and $D \in {^r}\mathbb R^n$.
	The size of $X = \begin{bmatrix} A & C \end{bmatrix}$ is ${^{2m}}\mathbb R^{r+k}$. The size of $Y = \begin{bmatrix} B & -D \end{bmatrix}$ is ${^{r+k}}\mathbb R^{2n}$.
	Since the inner dimensions of these augmented matrices match, we can perform matrix multiplication.
