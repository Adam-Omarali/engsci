...
def is_seq_complete(board, col, y_start, x_start, length, d_y, d_x):
    ...
    y = y_start
    x = x_start
    while is_sq_in_board(board, y, x) and count < length:
        if board[y][x] == col:
            count +=1
        else:
            return False
    ...
    #since already incremented and checking the next square, see if the length extends
    if board[y][x] == col:
        return False
    #check square in the opposite direction of start
